...
def pivot_months_pandas(data: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    grouped_data_monthly = data.groupby(["name", "month"]).sum().reset_index()
    monthly = grouped_data_monthly.pivot(index='name', columns='month', values = 'precipitation')

    grouped_data_counts = data.groupby(["name", "month"]).count().reset_index()
    counts = grouped_data_counts.pivot(index='name', columns='month', values = 'precipitation')
...
def compute_pairwise(df: pd.DataFrame, func: callable) -> pd.DataFrame:
    ...
    **chaque paire de lignes du dataframe**.
    ...
    ```
    """
    return pd.DataFrame(squareform(pdist(df, metric = func)), index = df.index, columns = df.index)
...
def compute_pairwise_distances(df: pd.DataFrame) -> pd.DataFrame:
    ...
    new_df = df[['name','latitude', 'longitude']]
    new_df = new_df.drop_duplicates().set_index('name').sort_index()
...
def correlation(u, v) -> float:
    ...
    a = (new_u - np.mean(new_u)) * (new_v - np.mean(new_v))
    b = np.std(new_u) * np.std(new_v)
...
def compute_pairwise_correlation(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    # TODO: faites pivoter le dataframe de sorte que vous ayez une colonne pour chaque date, et les noms des stations sont les indices.
...
def compute_pairwise_correlation_pandas(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    new_df = df.pivot(index='date', columns='name', values = 'precipitation')
